...
function submitMove (game, request, callback) {
    var response = {request: request,
		    moves: {},
		    active: true};
    response.moves[request.piece] = request.to;
    var STARTIDX = 1 << 25;
	...
	response.moves[request.piece] = request.from;
	callback(response);
	return;
    }
	...
	[function loadGameState (next) {
	 ...
	 function viewDiffs (game, response, next) {
		 ...
		 if (oldX[key] !== game.x[key]) {
		     response.moves[key] = game.x[key];
		 }
	     }
	     for (key in oldO) {
		 if (oldO[key] !== game.o[key]) {
		     response.moves[key] = game.o[key];
		 }
	     }
	 ...
	 function alertOpponent (game, response, next) {
	     ...
	     console.log('Right now X is:', bits.getFlags(response.x));
	     console.log('Right now O is:', bits.getFlags(response.o));
