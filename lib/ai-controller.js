...
function calculateMove(root, game, depth) {
	...
	var childScore = alphaBetaSearch(child.player, child.game,
...
function alphaBetaSearch (state, game, depth, alpha, beta, maximize) {
	...
	// Endgame state - win or lose
	return score(state, game, maximize);
    }

    while (branch = children.pop()) {
	var node = branch.player;
	var nodeGame = branch.game;
	var opponent = node ^ nodeGame;
	var branchScore = score(branch, game);
	var childScore;

	// Early cutoff on dead-end path
	if ((maximize && branchScore < alpha)
	    || (!maximize && branchScore > beta) ) {
	    continue;
	}
	
	childScore = alphaBetaSearch(opponent, nodeGame, depth -1,
				       alpha, beta, maximize);
